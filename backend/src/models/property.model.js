...
async function listProperties({ city, country, min_price, max_price, type, sort, page = 1, page_size = 20, viewer_id = null } = {}) {
  ...
  if (offset >= 100) {
    // beyond hard cap; return no rows
    const [empty] = await pool.query(sql + ' LIMIT 0');
    return empty;
  }
...
/** Delete a property owned by the given owner */
async function deleteProperty({ id, owner_id }) {
